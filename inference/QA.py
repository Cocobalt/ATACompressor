...
class ATALoraQAInfer(nn.Module):
    def __init__(self, llama_path, max_length, lora_config, num_mem, lora_path, max_output_length=600, max_chunks=6, 
        ...
        """
        ATALoraQAInfer is a model for inference that integrates LoRA (Low-Rank Adaptation) with memory networks.
        ...
        """
        super(ATALoraQAInfer, self).__init__()
    ...
    def forward(self, input_ids, labels, target, query_length):
        """
        Forward pass for the ATALoraQAInfer model. Performs inference with memory handling.

        Args:
            input_ids (Tensor): Input token IDs.
            labels (Tensor): Target labels for evaluation.
            target (Tensor): Ground truth token IDs.
            query_length (int): Length of the query tokens.

        Returns:
            dict: The inference results, including generated text and loss.
        """
        text_tokens = input_ids[:, :self.max_length]
        qa_tokens = input_ids[:, self.max_length:self.max_length + query_length]
        ...
        for _ in range(max_new_tokens):
            ...
            with self.llama.disable_adapter():
                output = self.llama(inputs_embeds=input_embeddings, past_key_values=trimmed_past_key_values)
            logits = output.logits
            next_token = torch.argmax(logits[:, -1, :], dim=-1)
            trimmed_past_key_values = output.past_key_values
        ...
        all_logits = torch.cat(all_logits, dim=1) if all_logits else None
        loss = None
        if labels is not None and all_logits is not None:
            labels = labels[:, :all_logits.size(1)]
            loss = self.criterion(all_logits.view(-1, all_logits.size(-1)), labels.view(-1))

        results = [{
            "loss": loss.item() if loss is not None else None,
            "generated_text": generated_text,
            "predicted_length": float(predicted_lengths[0].item()),
            "num_mem": int(dynamic_num_mem[0].item())
        }]
        ...
        return {"generated_text": generated_text, "loss": loss}
